...
`schema.yaml` is the **allowlist** that the GroundedQL compiler enforces. Only tables and columns declared here are reachable by the LLM or any QueryPlan. Anything not listed causes the compiler to raise a `QueryPlanError` — there is no silent fallthrough, no dynamic DB introspection.
...
If your physical table or column name contains uppercase letters, spaces, or reserved words, GroundedQL will automatically double-quote it in the generated SQL. You can also force quoting by wrapping the name in the YAML string:
...
!!! warning "`on:` must always be quoted"
    `on` is a YAML 1.1 reserved word that parses as boolean `true`. Always write `"on":` (with double quotes). GroundedQL's schema validator will raise a `SchemaError` with a helpful message if it detects this issue.
...
When `execute_query_plan` or `load_and_validate_schema` is called, GroundedQL validates the schema file and:
...
```python
from groundedql.api.api import load_and_validate_schema
