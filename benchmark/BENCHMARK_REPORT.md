# IntentQL Benchmark Report
## A Fair Head-to-Head Comparison: IntentQL vs LangChain SQLAgent vs GPT-4 Direct
...
> **GPT-4 Direct works in your demo and fails silently in production. IntentQL is the compiler
...
**IntentQL is what those teams wish they had at week one.**
...
IntentQL's compiler layer provides exactly those guarantees:
...
| Segment | The Pain | What IntentQL Provides |
|---|---|---|
...
GPT-4 Direct is IntentQL's most credible competitor and the benchmark treated it with maximum
...
models update, edge cases slip through. IntentQL's 50/50 score does not fluctuate — it is
...
error and no warning. IntentQL rejected all 15 at the allowlist before SQL was generated.
...
executes depends on the database driver. IntentQL sends all values as bind parameters — the
...
structured record of what was requested, no way to explain the reasoning. IntentQL generates
...
IntentQL solves this by never generating unsafe SQL in the first place. The allowlist check
...
IntentQL is a compiler that already handles this correctly. The build-vs-buy calculation is
not close.
...
IntentQL is not a retrieval system. It is a compiler. It handles any question that can be
...
### IntentQL is LLM-agnostic

IntentQL works with any LLM — OpenAI, Anthropic, Google Gemini, Azure OpenAI, or any
...
**For customers with self-hosted or on-premise LLMs, IntentQL's token cost is effectively
zero per query.**
...
Run once per schema change: `python -m intentql.spec_builder --schema config/schema.yaml`
...
| Deployment | IntentQL cost per 10 questions | GPT-4 Direct cost per 10 questions |
|---|---|---|
...
**1. Test cases were written by the IntentQL team.**
All adversarial inputs, hallucination inputs, and pipeline questions were authored by the
same team that built IntentQL. Independent replication is encouraged. All test data is published
in `benchmark/data/` and the Northwind database is publicly available.

**2. Determinism benchmark — two levels, one user-facing.**
Benchmark 2a tests the IntentQL compiler in isolation (no LLM) — an internal architectural
...
**3. Full-pipeline determinism is 4/5 for both IntentQL and GPT-4 Direct.**
At temperature=0, both tools produce non-deterministic output on semantically ambiguous
questions. The difference: when IntentQL's planner varies, the compiler still enforces schema
...
IntentQL and GPT-4 Direct return structured rows verified against known correct answers.
...
| Benchmark | IntentQL | LangChain ✱ | GPT-4 Direct |
|---|---|---|---|
...
|---|---|---|---|---|
| IntentQL | 14,481ms | 70,783 | 1.0 | $0.36 |
| LangChain | 26,435ms | 99,827 | 8.2 | $0.54 |
| GPT-4 Direct | 2,500ms | 16,180 | 1.0 | $0.08 |

**The headline results:**
- IntentQL and GPT-4 Direct both scored 10/10 on pipeline correctness.
- IntentQL is the only tool with stable 50/50 injection resistance across all runs.
...
- GPT-4 Direct is faster and cheaper. IntentQL's cost premium buys architectural safety guarantees.
...
|---|---|---|---|
| **IntentQL** | **50/50** | **0.0ms** | **0.3ms** |
| LangChain | 21/50 | 4,491ms | 10,822ms |
| GPT-4 Direct | 48/50 | 2,835ms | 6,392ms |

### Why IntentQL's 50/50 is not the same as GPT-4's 48/50

GPT-4's safety is enforced by a system prompt. IntentQL's 50/50 is structural — enforced
...
injection values are embedded as literals. IntentQL generates:
...
|---|---|---|---|
| IntentQL | 4/5 | 14,218ms | Includes LLM planner |
| LangChain | 3/5 | 12,975ms | Natural language output compared |
| GPT-4 Direct | 4/5 | 3,013ms | SQL string compared |

### Analysis

IntentQL and GPT-4 Direct tied at 4/5. The 1 IntentQL failure was question 5 — "orders grouped
...
**The critical difference:** When IntentQL's planner produces a different plan, the compiler
...
| Tool | Score | Avg Latency | P95 Latency |
|---|---|---|---|
| **IntentQL** | **15/15** | **0.0ms** | **0.1ms** |
| LangChain | 4/15 | 11,907ms | 23,929ms |
| GPT-4 Direct | 13/15 | 2,381ms | 5,192ms |

### Analysis

IntentQL catches hallucinations in the compiler's allowlist check — before SQL is generated,
...
| Tool | Score | Avg Latency | P95 Latency |
|---|---|---|---|
| **IntentQL** | **10/10 ✅** | **14,481ms** | **14,874ms** |
...
|---|---|---|---|---|
| IntentQL | 70,783 | 7,078 | 1.0 | $0.36 |
| LangChain | 99,827 | 11,092 | 8.2 | $0.54 |
| GPT-4 Direct | 16,180 | 1,618 | 1.0 | $0.08 |

**Reading the cost numbers honestly:**

GPT-4 Direct scored 10/10 on this run and is 6x cheaper and 6x faster than IntentQL.
...
(15/15 vs 13/15), and — most importantly — the architectural guarantee: IntentQL's safety
...
**IntentQL wins on safety — consistently, architecturally:**
- 50/50 injection resistance vs GPT-4's 48/50 — and IntentQL's score cannot fluctuate
...
**IntentQL ties on correctness and determinism:**
...
inputs and no hallucinations, GPT-4 Direct performs as well as IntentQL. The difference
appears at the edges — injection attempts, plausible hallucinations, model updates,
prompt drift. IntentQL's compiler catches those failures before they reach the database.
GPT-4 Direct does not.

In regulated environments, "catches most failures most of the time" is not enough.
The floor matters. IntentQL is the floor.

---

## The LLM Cost Argument: IntentQL Commoditizes the Model Requirement

### The key insight

Without IntentQL, the LLM does everything — understands the question, writes SQL, enforces
safety, handles edge cases. That requires an expensive, capable model.

With IntentQL, the LLM only needs to map a question to a structured JSON object. The compiler
...
|---|---|---|---|---|---|
| **IntentQL + gpt-4o-mini** | **50/50 ✅** | **15/15 ✅** | 10/10 | **$0.019** | 4,302ms |
| GPT-4 Direct + gpt-4o | 48/50 ❌ | 13/15 ❌ | 10/10 | $0.083 | 1,612ms |
| GPT-4 Direct + gpt-4o-mini | 50/50 ✅ | 11/15 ❌ | 10/10 | $0.004 | 1,753ms |

### What this proves

**IntentQL on a cheap model is safer than GPT-4 Direct on an expensive model.**
- 50/50 injection resistance vs 48/50 — and IntentQL's score is architectural, not prompt-dependent
...
Safety in IntentQL does not change — it is enforced by the compiler regardless of which LLM runs.
...
With IntentQL, using a cheaper model reduces cost without reducing safety. The compiler's
...
and government where data cannot leave the network), IntentQL's marginal token cost is
