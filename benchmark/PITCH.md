...
IntentQL's job is to compile that plan into safe, parameterized SQL — enforcing your schema,
...
User question
     │
     ▼
  LLM planner  ──→  QueryPlan JSON  ──→  IntentQL Compiler  ──→  Parameterized SQL  ──→  Database
...
IntentQL's marginal token cost is effectively zero.
...
|---|---|---|---|
| **IntentQL** | **50/50** | **15/15** | **Yes — enforced in code** |
...
IntentQL rejected all injection attempts and all hallucinations in the compiler — before
...
|---|---|---|
| **IntentQL** | **10/10** | **Yes — row counts verified** |
| GPT-4 Direct | 10/10 | Yes — row counts verified |
| LangChain | 9/9 completed | No — natural language output only |

Both IntentQL and GPT-4 Direct scored 10/10 on the Northwind pipeline benchmark. This is
...
model updates, and adversarial inputs. IntentQL's compiler catches those before they reach
...
| IntentQL | 14,481ms | $0.36 (GPT-4o) / ~$0 (self-hosted LLM) |
...
industries where data cannot leave the network — IntentQL's marginal token cost is zero.
LangChain is the most expensive tool at $0.54 per 10 questions with the worst safety
scores of any tool tested.

---

## IntentQL Commoditizes the LLM Requirement

This is the argument that changes the cost conversation completely.

Without IntentQL, the LLM must write safe, correct, parameterized SQL. That requires
a powerful, expensive model. Even then it fails — as the benchmark shows.

With IntentQL, the LLM only needs to map a question to a structured JSON object.
...
|---|---|---|---|
| **IntentQL + gpt-4o-mini** | **15/15 ✅** | 10/10 | **$0.019** |
| GPT-4 Direct + gpt-4o | 13/15 ❌ | 10/10 | $0.083 |
| GPT-4 Direct + gpt-4o-mini | 11/15 ❌ | 10/10 | $0.004 |

**IntentQL on a cheap model is safer than GPT-4 Direct on an expensive model — and 4x cheaper.**
...
degrades as the model gets weaker. Safety in IntentQL does not change — it is enforced
by the compiler, not the model.

For organizations running self-hosted or on-premise LLMs, IntentQL's marginal cost is
...
are liability, not bugs. IntentQL's schema allowlist is a documented, auditable policy —
...
IntentQL's compiler is a pure function: same plan, same SQL, always.

**You are building a multi-tenant analytics product.**
User A must never see User B's data. GPT-4 has no tenant isolation layer. IntentQL's schema
...
change behavior you relied on. IntentQL's compiler is the floor that catches those failures
...
residual risk, no audit trail, no structured error codes. IntentQL already exists.

---

## The Honest Limitations

**IntentQL costs more per query on OpenAI deployments.**
GPT-4 Direct at $0.08 vs IntentQL at $0.36 for 10 questions. This gap closes with
domain-scoped spec generation, prompt caching, and a cheaper planning model.
For self-hosted LLMs, the gap is zero.

**IntentQL is slower than GPT-4 Direct.**
...
processing steps. Switching to GPT-4o-mini brings IntentQL's latency to 2–4 seconds,
...
Both tools have LLM-driven variation on ambiguous questions. The difference: IntentQL's
...
> GPT-4 Direct is not selling you a seatbelt. IntentQL is.
...
IntentQL's compiler catches those before they reach the database.
